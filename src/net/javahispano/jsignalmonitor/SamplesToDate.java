 ...
 * determinada muestra. Esta clase se ha dise�ado mediante el patr�n Singleton,
 * se configura pas�ndole una fecha de nase y probee m�todos para saber dada una
...
//  static ResourceBundle res =ResourceBundle.getBundle("pintar.i18n.Res",Configuracion.localidad);
  private static boolean isInstancia = false;
  private static SamplesToDate instancia;
  private SimpleDateFormat parserFechaCompleta;
  private SimpleDateFormat parserFechaHora;
  ...
  private SamplesToDate() throws Exception {
    if (isInstancia) {
      ...
      isInstancia = true;
      parserFechaCompleta = new SimpleDateFormat("HH:mm:ss dd/MM/yyyy");
      parserFechaHora = new SimpleDateFormat("HH:mm:ss:SSS");
    }
  }

  /**
   * Data una fecha la toma como fecha base para todos os calculos que se le pidan
   * a continuaci�n: es la fecha d�nde se empieza a monitorizar.
  ...
  public boolean setFechaBase(String _fecha) {
    try {
      fecha_base = parserFechaCompleta.parse(_fecha);
      return true;
    }
  ...
  public String getFechaBase() {
    return parserFechaCompleta.format(fecha_base);
  }
   ...
   * @param fs frecuencia a la cual est� muestreada la se�al
   * @return nuero de muestras transcurridas. Si hubo alg�n fallo en la oporacion
   * devuelve Long.MIN_VALUE
   */
  public long getSamplesTill(String fecha_hasta, float fs) {
    Date date_fecha_hasta = null;
    boolean fallo = false;
    //En este caso la fecha est� en formato corto
    if (fecha_hasta.length() < 9) {
      ...
      Calendar calendario_fech_base = parserFechaCompleta.getCalendar();
      date_fecha_hasta = new Date(calendario_fech_base.get(Calendar.YEAR) -
          1900,
          calendario_fech_base.get(Calendar.MONTH),
          calendario_fech_base.get(Calendar.DAY_OF_MONTH), hora, minutos,
          segundos);
    }
    ...
    //Si no es que la fecha era completa
      ...
      Calendar tmp = null;
      try {
        tmp = (Calendar) parserFechaCompleta.getCalendar().clone();
        date_fecha_hasta = parserFechaCompleta.parse(fecha_hasta);
      }
      catch (ParseException ex) {
        ex.printStackTrace();
        fallo = true;
      ...
      //esta linea ha de ejecutarse tanto como si hay excepci�n como si no
      parserFechaCompleta.setCalendar(tmp);
    }
    if (!fallo) {
      Date date_fecha_base = parserFechaCompleta.getCalendar().getTime();
   ...
   * muestras de una se�al con una frecuencia de muestreo fs. La calcula sobre
   * la fecha base con la que se configur� este SamplesToDate.
   * Seg�n el valor del ultimo boolean se devolver� la fecha con o sin milisegundos.
   * @param muestras
   * @param fs
   * @param corta
   ...
   */
  public String getFecha(long muestras, float fs, boolean corta) {
    Date fecha_respuesta = (Date) parserFechaCompleta.getCalendar().getTime().
        clone();
    long tiempo_total = fecha_respuesta.getTime() +
        (long) (muestras * 1000 / fs);
    fecha_respuesta.setTime(tiempo_total);
    if (corta) {
      Calendar tmp = (Calendar) parserFechaCompleta.getCalendar().clone();
      String tmp2 = parserFechaHora.format(fecha_respuesta);
      parserFechaCompleta.setCalendar(tmp);
      return tmp2;
    }
    else {
      /*            Calendar tmp = (Calendar) parser_fecha_hora_completa_milisegundos.getCalendar().clone();
       ...
       parser_fecha_hora_completa_milisegundos.setCalendar(tmp);
              return tmp2;*/
      Calendar tmp = (Calendar) parserFechaCompleta.getCalendar().clone();
      String tmp2 = parserFechaCompleta.format(fecha_respuesta);
      parserFechaCompleta.setCalendar(tmp);
      return tmp2;
  ...
  private static final String texto_adevertencia_fecha =
      ...
      "</font><font color=\"#0000FF\"> Ej: 22:30:00 ser�an las ocho</font></font></p>" +
      "<p><font size=\"4\">&nbsp;&nbsp; <font color=\"#0000FF\">&nbsp;&nbsp;&nbsp;&nbsp; y media de la " +
      "tarde. Al no indicar ni a�o, ni d�a ni mes se supone que</font></font></p>" +
      "<p><font size=\"4\">&nbsp;&nbsp; <font color=\"#0000FF\">&nbsp;&nbsp;&nbsp;&nbsp; son los del " +
      "inicio de la monitorizaci�n.</font></font></p>" +
      ...
      "<p><font size=\"4\"><font color=\"#0000FF\"> &nbsp;&nbsp;&nbsp;&nbsp; Ej: 22:30:00:23 ser�an las ocho </font></font><font size=\"4\"><font color=\"#0000FF\"> " +
      ...
      "son los del inicio de la monitorizaci�n.</font></font></p>" +

      "<p><font size=\"4\">&nbsp;&nbsp; <font color=\"#0000FF\"> 3)Formato Largo </font><font color=\"#009933\">" +
      "hora:minutos:segundos a�o/mes/dia</font></font></p>" +
